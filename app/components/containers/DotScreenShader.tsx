...
const DotMaterial = shaderMaterial(
  ...
  /* glsl */ `
    ...
    void main() {
      ...
      // Calculate dot visibility with mouse influence
      float dotVisibility = smoothDot * combinedMask * dotOpacity * (1.0 + opacityInfluence);
      
      // Only show dots, make everything else transparent
      vec3 finalColor = dotColor;
      float alpha = dotVisibility;
...
function Scene({ theme = 'light' }: SceneProps) {
  ...
  useEffect(() => {
    ...
    // bgColor is not used anymore since we use transparency
...
type DotScreenShaderProps = {
  theme?: 'light' | 'dark'
}

export const DotScreenShader = ({ theme = 'light' }: DotScreenShaderProps) => {
  return (
    <Canvas
